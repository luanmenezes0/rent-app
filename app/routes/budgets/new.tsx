...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const [clients, rentables] = await Promise.all([
    getClients({}),
    getRentables(),
  ]);
...
export default function NewBudget() {
  const { clients, rentables } = useLoaderData<typeof loader>();
  ...
  useEffect(() => {
    async function loadBuildingSites() {
      if (selectedClient) {
        const response = await fetch(
          `/api/building-sites?clientId=${selectedClient}`,
        );
        const buildingSites = await response.json();
